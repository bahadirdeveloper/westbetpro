  ...
  return (
    <header className="glass-nav border-b border-white/5 sticky top-0 z-40 px-3 sm:px-4 md:px-6 lg:px-8 py-3 flex items-center justify-between gap-2">
      <div className="flex items-center gap-2 min-w-0 flex-shrink">
        ...
        <span className="text-[10px] sm:text-xs font-bold text-primary tracking-widest uppercase truncate">{statusText}</span>
      </div>
      ...
      <div className="flex items-center gap-2 sm:gap-3 md:gap-4 flex-shrink-0">
        <div className="relative group hidden lg:block">
          ...
          <input
            className="bg-white/5 border border-white/10 rounded-full pl-10 pr-4 py-2 text-sm focus:ring-1 focus:ring-primary focus:border-primary w-56 xl:w-64 outline-none"
        ...
        <Link
          href="/admin/login"
          className="flex items-center gap-1.5 px-2.5 sm:px-3 py-1.5 sm:py-2 bg-saddle-brown/20 hover:bg-saddle-brown/30 text-aged-gold border border-aged-gold/20 rounded-lg transition-all"
        >
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          <div className="text-right hidden md:block">
            <p className="text-xs font-bold">Admin Serifi</p>
            <p className="text-[10px] text-aged-gold">Vahsi Bati Plani</p>
          </div>
          <img
            alt="User Avatar"
            className="w-8 h-8 sm:w-10 sm:h-10 rounded-full border border-aged-gold flex-shrink-0"
